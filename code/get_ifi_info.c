...
struct ifi_info *get_ifi_info(int family, int doaliases)
{
	...
	char *ptr, *buf, lastname[IFNAMSIZ], *cptr, *haddr, *salname;
	struct ifconf ifc;
	...
	if (ifr->ifr_addr.sa_family == AF_LINK) {
		struct sockaddr_dl *sdl = (sockaddr_dl *)&ifr->ifr_addr;
		...
		*ifipnext = ifi; /* prev points to this new one */
		...
		switch(ifr->ifr_addr.sa_family) {
			case AF_INET:
					...
					break;
			case AF_INET6:
					sin6ptr = (struct sockaddr_in6 *)&ifi->ifr_addr;
